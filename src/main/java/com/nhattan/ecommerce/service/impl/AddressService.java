...
public class AddressService implements IAddressService {
	...
	@Transactional
	@Override
	public AddressDTO save(AddressDTO addressRequest) {
		if (!customerRepository.exists(addressRequest.getCustomer().getCustomerID()))
			throw new NotFoundException("customer-not-found");

		int MaxNumberAddress = 10;
		if (addressRepository.countAddressByCustomerID(addressRequest.getCustomer().getCustomerID()) >= MaxNumberAddress)
	...
	@Transactional
	@Override
	public AddressDTO update(AddressDTO addressRequest) {
		...
		return modelMapper.map(addressRepository.findOne(addressRequest.getAddressID()), AddressDTO.class);
	}
	...
	public List<AddressDTO> showAll(int customerID) {
		...
		return addressRepository.findByCustomerID(customerID).stream()
				.map(x -> modelMapper.map(x, AddressDTO.class)).collect(Collectors.toList());
	}
